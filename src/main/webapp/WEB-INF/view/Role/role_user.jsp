...
<body>
	<form name="form1" method="post" action="right">
		...
		<input type="hidden" name="tableName" value="d_user_role">
		...
		<tr>
			...
			<tr>
				<td width="40%">不是该角色的成员</td>
				<td width="20%"></td>
				<td width="40%">是该角色的成员</td>
			</tr>
			<tr>
				<td width="40%">
					<select style="width:100%;" multiple id = "list1"  size="20" 
						...
						<c:forEach var="item" items="${user_name }">
							<option value="${item.getUser_id() }">${item.getUser_name() }</option>
						</c:forEach>
					</select>
				</td>
