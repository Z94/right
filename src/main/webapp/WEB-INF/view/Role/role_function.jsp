...
<body>
	<form name="form1" method="post" action="right">
		<input type="hidden" name="id" value="${role.getRole_id() }">
		<input type="hidden" name="tableName" value="d_r_role_fun">
		
		<input type="hidden" name="deleteName" value="role_id">
		<table align="center">
		<tr>
			...
			<tr>
				<td width="40%">本角色未拥有的功能</td>
				<td width="20%"></td>
				<td width="40%">本角色已拥有的功能</td>
			</tr>
			<tr>
				<td width="40%">
					<select style="width:100%;" multiple id = "list1"  size="20" ondblclick="moveOption(document.getElementById('list1'),document.getElementById('list2'))">	
						<c:forEach var="item" items="${function_name }">
							<option value="${item.getFunction_id() }">${item.getFunction_name() }</option>
						</c:forEach>
					</select>
				</td>
